...
function Draw()
	...
	-- notes display
	gui.text(kb.x+203, kb.y+1, "S1: "..channels.Square1.semitone..channels.Square1.octave,   "#ff0000ff", "#000000ff")
	gui.text(kb.x+203, kb.y+9, string.format("%.1f",channels.Square1.freq),   "#ff0000ff", "#000000ff")
	gui.text(kb.x+203, kb.y+17,  "S2: "..channels.Square2.semitone..channels.Square2.octave,   "#00b0ffff", "#000000ff")
	gui.text(kb.x+203, kb.y+25, string.format("%.1f",channels.Square2.freq),   "#00b0ffff", "#000000ff")
	gui.text(kb.x+203, kb.y+33, "TR: "..channels.Triangle.semitone..channels.Triangle.octave, "#00ff80ff", "#000000ff")
	gui.text(kb.x+203, kb.y+41, string.format("%.1f",channels.Triangle.freq),   "#00ff80ff", "#000000ff")
	gui.text(kb.x+203, kb.y+49, "NS: "..channels.Noise.freq, "#f0d000ff", "#000000ff")
	gui.text(kb.x+203, kb.y+57, string.format("%.1f",snd.rp2a03.noise.frequency),   "#f0d000ff", "#000000ff")
	...
	if (kb.on) then	
		...
		for name, chan in pairs(channels) do
			if name == "Square1" or name == "Square2" or name == "Triangle" then
				if name == "Triangle" then color = "#00ff80ff"
...
---------------------
	...
	for name, chan in pairs(channels) do
		if name == "Square1" or name == "Square2" then
			...
			if chan.duty == 0 then table.insert(chan.color,1,"#b0f0b0ff")
			elseif chan.duty == 1 then table.insert(chan.color,1,"#00ff00ff")
			elseif chan.duty == 2 then table.insert(chan.color,1,"#00f0f0ff")
		...
		elseif name == "Noise" then
			if chan.duty == 1 then
				...
				table.insert(chan.color,1,"#f0d000ff")
			end
		end
		-- draw volumes
		...
		if iterator <=14 then
			...
			gui.text(chan.x, chan.y+9+1, chan.vol[1], "#ffffffff", "#000000ff")
			if tonumber(chan.vol[1]) > 0 then
				for j = 0, chan.vol[1]-1 do
					gui.box(chan.x+13+j*2, chan.y+9, chan.x+15+j*2, chan.y+8+9, "#000000ff")
					gui.line(chan.x+14+j*2, chan.y+1+9, chan.x+14+j*2, chan.y+7+9, "#c0e0ffff")
			...
			for i = 1, #chan.vol do
				gui.text(chan.x, chan.y+i*9+1, chan.vol[i], "#ffffffff", "#000000ff")
				if tonumber(chan.vol[i]) > 0 then
					for j = 0, chan.vol[i]-1 do
						gui.box(chan.x+13+j*2, chan.y+i*9, chan.x+15+j*2, chan.y+8+i*9, "#000000ff")
						gui.line(chan.x+14+j*2, chan.y+1+i*9, chan.x+14+j*2, chan.y+7+i*9, "#c0e0ffff")
						if name == "Square1" or name == "Square2" or name == "Noise" then
							-- color comes from duty
							if name == "Square1" or name == "Square2" then
								numberp = 38
