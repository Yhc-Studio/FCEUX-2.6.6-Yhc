...
/*	Mappers:
176 - Standard
523 - Jncota KT-xxx, re-release of 灏佺姒滉瀴 浼忛瓟涓夊お瀛? 1 KiB->2 KiB, 2 KiB->4 KiB CHR, hard-wired nametable mirroring)
...
void Mapper176_Init(CartInfo *info) { /* .NES file */
	...
	else
	{
		...
		else
		{
			...
			if (ROM_size == 64 && VROM_size == 128) subType = 1; else
				if (ROM_size == 16 && VROM_size == 8) subType = 1; else
					if (ROM_size >= 512 && VROM_size == 0) subType = 2; else
						if (ROM_size == 256 && VROM_size == 0) subType = 3;
