...
static void power(void) {
	...
	if (maker == FFE) { /* For NARC (SM2006) and Best ¸‚ñR: Derby Stallion (SM4064), whose NMI vector point to the FFE BIOS' scratch RAM */
		WRAM[0x2032] = 0x4C;
...
static void cpuCycleBung(int a) {
	while (a--) {
		clockFDSCounter();
...
static void cpuCycleVenus(int a) {
	while (a--) {
		clockFDSCounter();
		if (tgdTarget & 0x8000) {
...
static void trapPPUAddressChangeFFE(uint32 A) {
...
static void trapPPUAddressChangeVenus(uint32 A) {
