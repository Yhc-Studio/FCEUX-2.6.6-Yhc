...
void NTDEC2291_Init(CartInfo *info) {
	submapper = info->submapper;
	Latch_Init(info, M63Sync, M63Read, 0x0000, 0x8000, 0xFFFF, 0);
}
/* -------------- Mapper 762 ------------------------ */
static void M762Sync(void) {
	if (latche & 0x0200)
		setprg32(0x8000, latche);
	else {
		setprg16(0x8000, (latche << 1) | (latche & 0x0100 ? 1 : 0));
		setprg16(0xC000, (latche << 1) | (latche & 0x0100 ? 1 : 0));
	}
	setchr8r(0x0000, 0);
	if (latche & 0x0080)
		setmirror(MI_H);
...
void Mapper762_Init(CartInfo *info) {
	Latch_Init(info, M762Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}

/*------------------ Map 748 ---------------------------*/
static void M748Sync(void) {
	int p = latche >> 3 & 0x07;
	int c = latche >> 1 & 0x03 | latche >> 2 & 0x0C;
	if (latche & 0x20) {
		setprg32(0x8000, p >> 1);
	}
	else {
	...
	setchr8(c);
	setmirror(latche & 0x01 ? MI_H : MI_V);
}

void Mapper748_Init(CartInfo *info) {
	Latch_Init(info, M748Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}
...
static void M763Sync(void)
{
	if (latche & 0x010) {
		setprg16(0x8000, latche << 1 & 0x1e | (latche & 0x020 ? 1 : 0));
		setprg16(0xc000, latche << 1 & 0x1e | (latche & 0x020 ? 1 : 0));
	}
	else {
		setprg32(0x8000, latche & 0xf);
	}
	setchr8(latche >> 6 & 1 | latche >> 7 & ~1);
	if (latche & 0x0080)
		setmirror(MI_H);
	else
		setmirror(MI_V);
...
void Mapper763_Init(CartInfo *info)
{
	Latch_Init(info, M763Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}

/* -------------- Mapper 743 ------------------------ */

static void M743Sync(void)
{
	if (latche & 8) {
		setprg16(0x8, latche);
		setprg16(0xC, latche);
...
void Mapper743_Init(CartInfo *info)
{
	Latch_Init(info, M743Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}

/* -------------- Mapper 734 ------------------------ */

static void M734Sync(void)
{
	if (latche & 0x1)
	{
		setprg32(0x8000, latche >> 2); // 32KB PRG
	}
	else
	{
		setprg16(0x8000, latche >> 1); // 16KB PRG at 0x8000
		setprg16(0xC000, latche >> 1); // 16KB PRG at 0xC000
	}

	// CHR-ROM Ó³Éä
	setchr8(latche >> 1); // 8KB CHR

	// ¾µÏñÉèÖÃ
	if (latche & 0x10)
		setmirror(MI_V); // ´¹Ö±¾µÏñ
	else
		setmirror(MI_H); // Ë®Æ½¾µÏñ
}

void Mapper734_Init(CartInfo *info)
{
	Latch_Init(info, M734Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}

/* -------------- Mapper 739 ------------------------ */

static void M739Sync(void)
{
	uint16 cpuA14 = !(latche & 0x001);
	uint16 mirrorH = !!(latche & 0x002);
	uint16 nrom = !!(latche & 0x100);
	uint16 chr = (nrom ? (latche >> 1 & 0xC | latche >> 5 & 0x3) : (latche >> 1 & 0x8 | latche >> 5 & 0x7));
	uint16 prg = (latche >> 2 & 0x07);
	setprg16(0x8000, prg & ~(cpuA14 * nrom));
	setprg16(0xC000, prg | cpuA14 * nrom | 7 * !nrom);
	setchr8(chr);
	if (mirrorH)
		setmirror(MI_H);
	else
...
void Mapper739_Init(CartInfo* info)
{
	Latch_Init(info, M739Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}
...
static void M732Sync(void) {
	...
	setchr8(latche);
	setmirror(latche & 8 ? MI_H : MI_V);
}

void Mapper732_Init(CartInfo* info) {
	Latch_Init(info, M732Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}
...
/*------------------ Map 731 ---------------------------*/
static void M731Sync(void) {
	if (latche & 1) {
		setprg32(0x8000, latche >> 5);
		setchr8(latche >> 3 & ~3 | latche >> 2 & 3);
	}
	else {
		setprg16(0x8000, latche >> 4);
		setprg16(0xC000, latche >> 4);
		setchr8(latche >> 3 & ~1 | latche >> 2 & 1);
	}
	if (latche & 2)
		setmirror(MI_H);
	else
...
void Mapper731_Init(CartInfo* info) {
	Latch_Init(info, M731Sync, NULL, 0x0000, 0x8000, 0xFFFF, 0);
}