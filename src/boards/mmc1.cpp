...
/* NES 2.0 Mapper 543 - 1996 無敵智カ�?5-in-1 (CH-501) */
...
static void M737PRG16(uint32 A, uint8 V) {
	if (reg & 0x20)
		setprg16(A, (V & 0x07) | (reg & 0x18));
	else
		if (reg & 0x04)
...
static void M737CHR4(uint32 A, uint8 V) {
	setchr4(A, (V & 0x1F) | reg << 2 & 0x60);
}

static DECLFW(M737Write) {
	reg = A & 0xFF;
	MMC1PRG();
	MMC1CHR();
}

static void M737Reset(void) {
	reg = 0;
	MMC1CMReset();
}

static void M737Power(void) {
	reg = 0;
	GenMMC1Power();
	SetWriteHandler(0x6000, 0x7FFF, M404Write);
}

void Mapper737_Init(CartInfo* info) {
	GenMMC1Init(info, 256, 256, 0, 0);
	info->Power = M737Power;
	info->Reset = M737Reset;
	MMC1CHRHook4 = M737CHR4;
	MMC1PRGHook16 = M737PRG16;
	AddExState(&reg, 1, 0, "REG");
}