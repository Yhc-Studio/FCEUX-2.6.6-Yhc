...
void syncPRG(int AND, int OR) {
	setprg8(0x8000, getPRGBank(0) &AND | OR);
	setprg8(0xA000, getPRGBank(1) &AND | OR);
	setprg8(0xC000, getPRGBank(2) &AND | OR);
	setprg8(0xE000, getPRGBank(3) &AND | OR);
}
void syncPRG_GNROM_66(int A14, int AND, int OR) {
	setprg8(0x8000, (getPRGBank(0) &~A14 &~1) &AND | OR);
	setprg8(0xA000, (getPRGBank(0) &~A14 | 1) &AND | OR);
	setprg8(0xC000, (getPRGBank(0) | A14 &~1) &AND | OR);
	setprg8(0xE000, (getPRGBank(0) | A14 | 1) &AND | OR);
}
...
void syncPRG_NROM(int AND, int OR, int bankAND, int cpuMask) {
	for (int bank = 0; bank <4; bank++)
		setprg8(0x8000 + bank * 0x2000, getPRGBank(bank &bankAND) &AND &~cpuMask | OR | bank &cpuMask);
}
...
int	getCHRBank(int bank) {
	...
	if (bank & 4)
		return DRegBuf[bank - 2];
	else
		return DRegBuf[bank >> 1] & ~1 | bank & 1;
};
void syncCHR_ROM(int AND, int OR) {
	for (int bank = 0; bank < 8; bank++) {
		int Temp = getCHRBank(bank);
		setchr1(bank * 1024, Temp &AND | OR);
	}
}
void syncCHR_RAM(int AND, int OR) {
	for (int bank = 0; bank <8; bank++)
		setchr1(bank * 1024, getCHRBank(bank) &AND | OR);
}
...
static void M315CW(uint32 A, uint8 V) {
	syncCHR_ROM(0xFF, ((EXPREGS[0] & 0x08) ? 0x040 : 0x000) | ((EXPREGS[0] & 0x02) ? 0x080 : 0x000) | ((EXPREGS[0] & 0x01) ? 0x100 : 0x000));
}
