...
void MSM6585_init(MSM6585* chip, int32 newHostClock, int (*newGetInput)(void)) {
	...
	AddExState(&chip->signal, 2, 0, "MSMO");
	AddExState(&chip->count, 4, 0, "MSMC");
	AddExState(&chip->rate, 4, 0, "MSMR");
	AddExState(&chip->step, 2, 0, "MSMS");
}

void MSM6585_reset(MSM6585* chip) {
	chip->count = 0;
	chip->step = 0;
	chip->signal = -2;
}
...
void MSM6585_run(MSM6585* chip) {
	chip->count += chip->rate;
	while (chip->count >= chip->hostClock) {
		chip->count -= chip->hostClock;
		uint8_t nibble;
		if (chip->whichNibble)
			nibble = chip->input & 0x0F;
		else {
			chip->input = chip->getInput();
			nibble = chip->input >> 4;
		}
		chip->whichNibble = !chip->whichNibble;
		chip->signal += diff_lookup[chip->step * 16 + nibble];
		if (chip->signal > 2047) chip->signal = 2047;
		if (chip->signal < -2048) chip->signal = -2048;
		chip->step += index_shift[nibble & 7];
		if (chip->step > 48) chip->step = 48;
		if (chip->step < 0) chip->step = 0;
	}
}

int32_t MSM6585_getOutput(MSM6585* chip) {
	return chip->signal;
}