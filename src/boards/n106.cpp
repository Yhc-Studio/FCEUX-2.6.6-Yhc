...
static INLINE int32 N163ApplyVolume(int32 v)
{
	return (int32)(((int64)v * FSettings.N163Volume) >> 8);
}
	...
	for (P = 7; P >= (7 - ((IRAM[0x7F] >> 4) & 7)); P--) {
		if ((IRAM[0x44 + (P << 3)] & 0xE0) && (IRAM[0x47 + (P << 3)] & 0xF)) {
			...
			for (V = CVBC << 1; V < (int)SOUNDTS << 1; V++) {
				WaveHi[V >> 1] += N163ApplyVolume((int32)duff2);
				if (!vco) {
					PlayIndex[P] += freq;
	...
	for (P = 7; P >= 7 - ((IRAM[0x7F] >> 4) & 7); P--) {
		if ((IRAM[0x44 + (P << 3)] & 0xE0) && (IRAM[0x47 + (P << 3)] & 0xF)) {
			...
			for (V = 0; V < Count * 16; V++) {
				...
				Wave[V >> 4] += N163ApplyVolume((int32)duff2);
				vco += 0x8000;
			}
			vcount[P] = vco;
		}
