...
static INLINE int32 VRC6ApplyVolume(int32 v)
{
	return (int32)(((int64)v * FSettings.VRC6Volume) >> 8);
}
...
static INLINE void DoSQV(int x) {
	int32 V;
	int32 amp = VRC6ApplyVolume(((((vpsg1[x << 2] & 15) << 8) * 6 / 8) >> 4));
	int32 start, end;
	...
	if (vpsg2[2] & 0x80) {
		...
		for (V = start; V < end; V++) {
			...
			rea:
				...
				if (saw1phaseacc <= 0)
					goto rea;
				duff = VRC6ApplyVolume((((phaseacc >> 3) & 0x1f) << 4) * 6 / 8);
			}
			Wave[V >> 4] += duff;
		}
	}
...
static INLINE void DoSQVHQ(int x) {
	int32 V;
	int32 amp = VRC6ApplyVolume(((vpsg1[x << 2] & 15) << 8) * 6 / 8);
...
static void DoSawVHQ(void) {
	...
	if (vpsg2[2] & 0x80) {
		for (V = cvbc[2]; V < (int)SOUNDTS; V++) {
			WaveHi[V] += VRC6ApplyVolume((((phaseacc >> 3) & 0x1f) << 8) * 6 / 8);
			vcount[2]--;
			if (vcount[2] <= 0) {
