...
// Digital Prime 64-color RGB palette, embedded for the NTSC filter only.
// This keeps ordinary FCEUX palette behavior unchanged while forcing NTSC 2x
// to use Digital Prime as its base color table.
static const unsigned char ntsc_digital_prime_base_palette[64 * 3] =
{
	...
	0x00, 0x6C, 0x97,  0x00, 0x00, 0x00,  0x00, 0x00, 0x00,  0x00, 0x00, 0x00,
...
int InitBlitToHigh(int b, uint32 rmask, uint32 gmask, uint32 bmask, int efx, int specfilt, int specfilteropt)
{
	...
	if(specfilt == 3) // NTSC 2x
	{
		...
		// Force NTSC 2x to use the Digital Prime 64-color palette as the
		// base color table, while still letting nes_ntsc generate composite
		// artifacts/fringing/bleed from that base palette.
		ntsc_setup.base_palette = ntsc_digital_prime_base_palette;
	...
	else if (specfilt >= 6 && specfilt <= 13)
	{
...
u32 ModernDeemphColorMap(const u8* src, const u8* srcbuf, int scale)
{
	...
	else if(scale == 13) return _ModernDeemphColorMap<13>(src, srcbuf);
	else if(scale == 14) return _ModernDeemphColorMap<14>(src, srcbuf);
...
static ModernDeemphColorMapFuncPtr getModernDeemphColorMapFunc(int scale)
{
	...
	else if(scale == 13) ptr = &_ModernDeemphColorMap<13>;
	else if(scale == 14) ptr = &_ModernDeemphColorMap<14>;
