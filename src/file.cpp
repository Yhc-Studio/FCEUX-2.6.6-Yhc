...
#ifdef _WIN32
static bool FCEU_IsValidUTF8(const std::string& text)
{
	if (text.empty())
		return true;

	return MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
		text.data(), static_cast<int>(text.size()), NULL, 0) > 0;
}
#endif

// Finds only a real archive separator. On Windows, legacy ANSI paths may use
// a DBCS code page such as CP936, where 0x7C can be the second byte of a
// Chinese character (for example, "Ù|" is D9 7C in GBK).
static size_t FCEU_FindArchiveSeparator(const std::string& text)
{
#ifdef _WIN32
	// UTF-8 continuation bytes are never 0x7C, so a normal search is safe.
	if (!FCEU_IsValidUTF8(text))
	{
		...
		for (size_t i = 0; i < text.size(); )
		{
			...
			if (IsDBCSLeadByteEx(codePage, ch) && i + 1 < text.size())
				i += 2;
			else
				++i;
		}
...
// Replaces only standalone archive separators, not a 0x7C trail byte inside
// a legacy Windows DBCS filename.
static std::string FCEU_ReplaceArchiveSeparators(std::string text, char replacement)
{
#ifdef _WIN32
	if (!FCEU_IsValidUTF8(text))
	{
		...
		for (size_t i = 0; i < text.size(); )
		{
			...
			else if (IsDBCSLeadByteEx(codePage, ch) && i + 1 < text.size())
			{
				i += 2;
			}
			else
...
{
	size_t pipe = FCEU_FindArchiveSeparator(src);
...
std::string FCEU_MakeFName(int type, int id1, const char* cd1)
{
	...
	return FCEU_ReplaceArchiveSeparators(ret, '.');
}
