...
#define INC32       (PPU[0] & 0x04)	//auto increment 1/32
 ...
 /*
  * FCEUPPU_MaskEnabled is a user-facing display option.  It must NOT force
  * PPU rendering on when the game has disabled $2001 BG/Sprite bits, because
  * many games blank rendering while updating VRAM or changing scroll.  Forcing
  * bits 3/4 on here makes the renderer fetch from unstable PPU state and causes
  * large blocks/garbage on games that intentionally blank the screen.
  *
  * Therefore:
  *   - Render/timing-sensitive bits keep the real $2001 value.
  *   - Display-only mask effects can be ignored when the option is disabled:
  *       grayscale/emphasis off, left 8px BG/Sprite clipping forced visible.
  */
#define PPUMASK_RENDER  (PPU[1])
#define PPUMASK_VISUAL  (FCEUPPU_MaskEnabled ? PPU[1] : ((PPU[1] & 0x18) | 0x06))

#define SpriteON    (PPUMASK_RENDER & 0x10)	//Show Sprite
#define ScreenON    (PPUMASK_RENDER & 0x08)	//Show screen
#define PPUON       (PPUMASK_RENDER & 0x18)	//PPU should operate
#define GRAYSCALE   (PPUMASK_VISUAL & 0x01)	//Grayscale (AND palette entries with 0x30)

#define SpriteLeft8 (PPUMASK_VISUAL & 0x04)
#define BGLeft8     (PPUMASK_VISUAL & 0x02)
...
// User-facing video option.  Enabled by default to preserve normal emulation.
// Disabling it ignores display-only PPU $2001 mask effects while preserving
// the real BG/Sprite render-enable bits to avoid corrupting PPU timing/state.
int FCEUPPU_MaskEnabled = 1;
...
#undef RefreshAddr
	...
	RefreshAddr = smorkus;
	if (firsttile <= 2 && 2 < lasttile && !BGLeft8) {
		uint32 tem;
		tem = READPAL(0) | (READPAL(0) << 8) | (READPAL(0) << 16) | (READPAL(0) << 24);
		tem |= 0x40404040;
...
static void DoLine(void) {
	...
	for (x = 63; x >= 0; x--)
		*(uint32*)&dtarget[x << 2] = ((PPUMASK_VISUAL >> 5) << 0) | ((PPUMASK_VISUAL >> 5) << 8) | ((PPUMASK_VISUAL >> 5) << 16) | ((PPUMASK_VISUAL >> 5) << 24);
