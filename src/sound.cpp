...
#define FCEU_NSF_MAX_EXP_SOUNDS 8

static EXPSOUND NSFExpSoundSlots[FCEU_NSF_MAX_EXP_SOUNDS];
static int NSFExpSoundCount = 0;
static int NSFExpSoundMuxEnabled = 0;

static void FCEU_NSFExpSoundMux_Fill(int Count)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].Fill)
			NSFExpSoundSlots[i].Fill(Count);
	}
}

static void FCEU_NSFExpSoundMux_NeoFill(int32* Wave, int Count)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].NeoFill)
			NSFExpSoundSlots[i].NeoFill(Wave, Count);
	}
}

static void FCEU_NSFExpSoundMux_HiFill(void)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].HiFill)
			NSFExpSoundSlots[i].HiFill();
	}
}

static void FCEU_NSFExpSoundMux_HiSync(int32 ts)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].HiSync)
			NSFExpSoundSlots[i].HiSync(ts);
	}
}

static void FCEU_NSFExpSoundMux_RChange(void)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].RChange)
			NSFExpSoundSlots[i].RChange();
	}
}

static void FCEU_NSFExpSoundMux_Kill(void)
{
	int i;
	for (i = 0; i < NSFExpSoundCount; i++)
	{
		if (NSFExpSoundSlots[i].Kill)
			NSFExpSoundSlots[i].Kill();
	}
}

void FCEU_NSFClearExpSounds(void)
{
	memset(NSFExpSoundSlots, 0, sizeof(NSFExpSoundSlots));
	NSFExpSoundCount = 0;
	NSFExpSoundMuxEnabled = 0;
	memset(&GameExpSound, 0, sizeof(GameExpSound));
}

void FCEU_NSFAddExpSound(const EXPSOUND* sound)
{
	if (!sound)
		return;

	if (!sound->Fill && !sound->NeoFill && !sound->HiFill && !sound->HiSync && !sound->RChange && !sound->Kill)
		return;

	if (NSFExpSoundCount >= FCEU_NSF_MAX_EXP_SOUNDS)
		return;

	NSFExpSoundSlots[NSFExpSoundCount++] = *sound;
}

int FCEU_NSFGetExpSoundCount(void)
{
	return NSFExpSoundCount;
}

void FCEU_NSFCommitExpSoundMux(void)
{
	EXPSOUND mux;

	if (NSFExpSoundCount <= 0)
	{
		memset(&GameExpSound, 0, sizeof(GameExpSound));
		NSFExpSoundMuxEnabled = 0;
		return;
	...
	if (NSFExpSoundCount == 1)
	{
		GameExpSound = NSFExpSoundSlots[0];
		NSFExpSoundMuxEnabled = 0;
		return;
	}

	memset(&mux, 0, sizeof(mux));
	mux.Fill = FCEU_NSFExpSoundMux_Fill;
	mux.NeoFill = FCEU_NSFExpSoundMux_NeoFill;
	mux.HiFill = FCEU_NSFExpSoundMux_HiFill;
	mux.HiSync = FCEU_NSFExpSoundMux_HiSync;
	mux.RChange = FCEU_NSFExpSoundMux_RChange;
	mux.Kill = FCEU_NSFExpSoundMux_Kill;

	GameExpSound = mux;
	NSFExpSoundMuxEnabled = 1;
}

int FCEU_NSFIsExpSoundMuxEnabled(void)
{
	return NSFExpSoundMuxEnabled;
}

void FCEU_NSFRefreshExpSoundMux(void)
{
	if (NSFExpSoundMuxEnabled)
...
int FlushEmulateSound(void)
{
	...
	if (FSettings.soundq >= 1)
	{
		...
		if (NSFExpSoundMuxEnabled)
			FCEU_NSFExpSoundMux_HiFill();
		else if (GameExpSound.HiFill)
			GameExpSound.HiFill();
		...
		if (NSFExpSoundMuxEnabled)
			FCEU_NSFExpSoundMux_HiSync(left);
		else if (GameExpSound.HiSync)
	...
	else
	{
		...
		if (NSFExpSoundMuxEnabled)
			FCEU_NSFExpSoundMux_Fill(end & 0xF);
		else if (GameExpSound.Fill)
...
void SetSoundVariables(void)
{
	...
	if (NSFExpSoundMuxEnabled)
		FCEU_NSFExpSoundMux_RChange();
	else if (GameExpSound.RChange)
