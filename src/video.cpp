...
void FCEU_PutImage(void)
{
	...
	{
		/*
		 * Normal games already call snapAVI() above before HUD/message overlays are drawn.
		 * NSF rendering takes a separate branch, so without this call AVI recording
		 * gets audio only unless HUD recording is enabled.  Record the already-drawn
		 * NSF player screen here while keeping the old non-HUD behavior for carts.
		 */
